...
const SuiteCard: React.FC<SuiteCardProps> = ({fotos, title, setView}) => {
  return (
    <section className='mb-16 rounded select-none border p-1 pe-0 flex flex-col sm:flex-row mx-auto sm:w-[560px] md:w-[700px] lg:w-[1000px] 2xl:w-[1100px] justify-between'>
      <Image onClick={() => setView(fotos, 0)} className='mx-auto sm:hover:scale-105 transition duration-75 rounded cursor-pointer sm:mx-0 w-11/12 sm:w-[250px] sm:h-[250px] lg:h-[400px] lg:w-[400px]' height={400} width={400} src={fotos[0]} alt='suite-casal' />
      <section className='flex mx-auto sm:mx-0 sm:ms-4 md:ms-8 w-full flex-col'>
        ...
        <section className='mx-auto w-11/12 sm:mx-0 '>
          ...
          <section className='ms-3 flex w-full flex-wrap lg:ms-6'>
            {fotos.map((foto, i) => <Image onClick={() => setView(fotos, i)} className='me-2 sm:hover:scale-110 transition duration-75 cursor-pointer mb-2 w-2/12 md:w-[48px] lg:w-[96px]' key={i} loading="eager" width={96} src={foto} alt='suite-casal' />)}
          </section>
        </section>
      </section>
    </section>
